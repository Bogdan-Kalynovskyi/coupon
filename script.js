function Coupon (options) {
    ...
    function load() {
        var compText = _("couponCodeTextBox"),
        ...
        _("mainImg").src = data.mainImg;
        _("footer").style.background = data.themeStyle.bgColor;
        _("couponCodeTextBox").style.background = data.themeStyle.buttonColor;
        _("couponCodeBlock").children[0].style.color = data.themeStyle.textColor;
        ...
        _("shareTitle").innerText = data.socialShare.overlayMessage;
        _("shareTitle").style.color = data.themeStyle.textColor;
        _("icon-facebook").href = "http://www.facebook.com/sharer.php?s=100&p[url]=" + shareValue + "&p[summary]=" + message;
        if (searchPosition === -1) {
            _("icon-twitter").href = "http://twitter.com/share?url=" + shareValue + "&text=" + message;
        }
        else
        {
            _("icon-twitter").href = "http://twitter.com/share?text=" + message.replace(shareValue, '') + "&url=" + shareValue;
        }
        _("icon-google").href = "https://plus.google.com/share?url=" + shareValue;


        if (data.socialShare.status === '1') {
            _("shareButtonsContainer").style.display = 'block';
        }
        else {
            _("shareButtonsContainer").style.display = 'none';
        }
        ...
        for (var i = 0; i < len; i++) {
            ...
            if (data.claimButtons[i].buttonType === 'site') {
                path = "<path id='icon-share' d='M 12 12 v -2 c 0 0 -8 -2 -9 2 c 0 -2 1 -6 9 -6 V 4 l 4 4.021 L 12 12 Z M 9 13 H 1 V 3 h 8 v 1 h 1 V 2 H 0 v 12 h 10 v -2 H 9 V 13 Z'></path>";
                action = "target='_blank' href='" + data.claimButtons[i].actionValue + "'";
                onclick[i] = downloadCoupon;
            }
            else if (data.claimButtons[i].buttonType === 'download') {
                path = "<path id='icon-inbox-download' d='M 8 9.4 l -4 -3.73333 h 2 v -4.66667 h 4 v 4.66667 h 2 L 8 9.4 Z M 13.867 4.73333 H 13 l 1.486 3.73333 H 11 c 0 1.54653 -1.342 2.8 -3 2.8 c -1.656 0 -3 -1.25347 -3 -2.8 H 1.514 L 3 4.73333 H 2.133 L 0 9.4 v 5.6 h 16 v -5.6 L 13.867 4.73333 Z'></path>";
                action = "target='_blank' href='" + data.claimButtons[i].actionValue + "'";
                onclick[i] = downloadCoupon;
            }
            else if (data.claimButtons[i].buttonType === 'code') {
                path = "<path id='icon-eye' d='M 8 3 C 2 3 0 8 0 8 s 2 5 8 5 s 8 -5 8 -5 S 14 3 8 3 Z M 8 12 c -4.366 0 -6.326 -2.938 -6.893 -4 C 1.676 6.935 3.637 4 8 4 c 4.366 0 6.326 2.938 6.893 4 C 14.324 9.065 12.363 12 8 12 Z M 10.121 5.879 C 10.664 6.422 11 7.172 11 8 c 0 1.657 -1.344 3 -3 3 S 5 9.657 5 8 s 1.344 -3 3 -3 v 3 L 10.121 5.879 Z'></path>";
                action = '';
                onclick[i] = showCode;
                var block = _("couponCodeBlock").children[0];
                block.innerText = data.claimButtons[i].overlayMessage;
                block.style.color = data.themeStyle.textColor;
                var text = _("couponCodeTextBox");
                text.innerText = data.claimButtons[i].actionValue;
                ...
                "<a id='button" + i +
                    ...
                    "' " + action + ">\
                        <svg fill='" + data.themeStyle.textColor + "'>" +
                            path +
                        "</svg>" +
                    data.claimButtons[i].buttonText +
                "</a>";

        }
        _("footer").innerHTML = html;

        for (var i = 0; i < len; i++) {
            _("button" + i).onclick = onclick[i];
        }

        if (data.socialShare.preClaim === "1") {
            _("overlay").style.display = 'block';
            _("shareBlock").style.display = 'block';
            _("button-close").style.display = 'none';
        }



        function downloadCoupon() {
            _("couponCodeBlock").style.display = 'none';
            _("overlay").style.display = 'block';
            _("shareBlock").style.display = 'block';
        }

        function showCode() {
            _("couponCodeBlock").style.display = 'block';
            _("overlay").style.display = 'block';

            if (postClaim === false) {
                _("shareBlock").style.display = 'none';
            }
        }

        _('button-close').onclick = function () {
            _("overlay").style.display = 'none';
        };

        _("couponCodeTextBox").onclick = function () {
        ...
        _('icon-facebook').onclick = _('icon-twitter').onclick = _('icon-google').onclick = function () {
            if (!postClaim) {
                _("overlay").style.display = 'none';
            }
            _("button-close").style.display = 'block';
        };
    }
    ...
    function render() {
        ...
        container.innerHTML =
        '<header><img id="mainImg"></header>\
        <footer id="footer"></footer>\
        <div id="overlay" class="overlay">\
            <svg fill="rgba(255, 255, 255, 1)" id="button-close">\
                <path id="icon-close"\
            ...
            <div class="centered">\
                <div id="couponCodeBlock">\
                    <div></div>\
                    <div id="couponCodeTextBox" class="coupon-code"></div>\
                </div>\
                <div id="shareBlock">\
                    <div id="shareTitle"></div>\
                    <div id="shareButtonsContainer"><a class="icon" id="icon-facebook" target="_window">\
                        <svg fill="#000000">\
                    ...
                    </a> <a class="icon" id="icon-twitter" target="_window">\
                        <svg fill="#000000">\
                    ...
                    </a> <a class="icon" id="icon-google" target="_blank">\
                        <svg fill="#000000">\
